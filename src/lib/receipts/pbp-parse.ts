...
/** Field-goal distance → Sleeper bucket key. */
...
export function deltasFor(
  ...
  if (r.kicker_player_id) {
    const d: Record<string, number> = {};
    if (r.field_goal_result === "made") d[fgKey(n(r.kick_distance))] = 1;
  ...
  if (r.return_touchdown === "1") {
    if (r.kickoff_returner_player_id)
      add(r.kickoff_returner_player_id, { kr_td: 1, kr_yd: n(r.return_yards) });
    if (r.punt_returner_player_id)
      add(r.punt_returner_player_id, { pr_td: 1, pr_yd: n(r.return_yards) });
  }
  ...
  if (r.defteam) {
    ...
    if (
      r.return_touchdown === "1" &&
      r.td_team === r.defteam &&
      !r.kickoff_returner_player_id &&
      !r.punt_returner_player_id
    ) {
      d.def_td = 1;
    }
    if (r.sp === "1" && r.posteam) d.pts_allow = n(r.posteam_score_post);
    if (Object.keys(d).length) out.push({ p: team(r.defteam), d });
  }
  if (r.sp === "1" && r.posteam && r.defteam) {
    // The offence's own DEF gives up nothing on this play, but its points-allowed
    // level must stay current when the other side scores on a return.
    out.push({ p: team(r.posteam), d: { pts_allow: n(r.defteam_score_post) } });
  }
  // One event per player per play: a rusher who also fumbled is one line.
  const merged = new Map<string, Record<string, number>>();
  for (const { p, d } of out) {
    const cur = merged.get(p) ?? {};
    for (const [k, v] of Object.entries(d)) cur[k] = k === "pts_allow" ? v : (cur[k] ?? 0) + v;
    merged.set(p, cur);
  }
