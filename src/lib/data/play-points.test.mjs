...
test("completion credits passer yards and receiver PPR + yards", () => {
  const p = play(
    "(Shotgun) C.Stroud pass short right to N.Collins to LV 38 for 11 yards (J.Bennett).",
    "Pass Reception",
    11,
  );
  const cs = credits(p);
...
test("rushing touchdown", () => {
  const p = play(
    "W.Marks left tackle for 20 yards, TOUCHDOWN. K.Fairbairn extra point is GOOD, Center-J.Weeks.",
    "Rushing Touchdown",
    20,
    true,
  );
...
test("incompletion and interception", () => {
  assert.equal(
    by(
      credits(play("C.Stroud pass incomplete deep left to N.Collins.", "Pass Incompletion", 0)),
      "1",
    ).points,
    0,
  );
  ...
  assert.equal(
    by(
      credits(
        play(
          "C.Stroud pass deep middle intended for N.Collins INTERCEPTED by T.Wilson at LV 20.",
...
test("fumble lost to the other team", () => {
  const cs = credits(
    play(
      "W.Marks right end to HOU 40 for 2 yards. W.Marks FUMBLES (T.Wilson), RECOVERED by LV-J.Hummel at HOU 40.",
...
test("field goals by distance", () => {
  assert.equal(
    by(
      credits(
        play(
          "K.Fairbairn 57 yard field goal is GOOD, Center-J.Weeks, Holder-T.Townsend.",
          "Field Goal Good",
          0,
          true,
        ),
  ...
  assert.equal(
    by(
      credits(
        play("K.Fairbairn 44 yard field goal is No Good, Wide Right.", "Field Goal Missed", 0),
      ),
      "5",
    ).points,
    -1,
...
test("punt returner yards, punter nothing", () => {
  ...
  const p = play(
    "A.Cole punts 45 yards to HOU 32, Center-T.Duzansky. J.Jackson pushed ob at HOU 40 for 8 yards (C.McGrone).",
    "Punt",
    null,
  );
