...
| 091  | Add a real `/about` marketing page, built from the locked mock | P2 | M | — | DONE `dfffb11` (2026-08-26, not pushed); new `src/routes/about.tsx` + `src/components/icons/brand-marks.tsx` (real Claude/OpenAI Simple Icons marks, GROK as text monogram), one-line `/about` link added to `GuestHome`; uses only semantic Tailwind tokens (`bg-bg`/`text-fg`/`text-muted`/etc.) — no new font, no hardcoded hex (only grep hit is the harmless `href="#features"` anchor, whose `#fea` substring false-positives the hex regex); `data-skin="console"` force-on-mount/restore-on-unmount verified live via `agent-browser` through real SPA client-side navigation (not full page reloads, which would've masked a leaked attribute) across all 4 required cases — default `null`→`"console"`→`null`, and a pre-set `"boxscore"`→`"console"`→restored `"boxscore"`; kept only the one real, thrice-verified Codex demo exchange, no second fabricated turn; `bun run typecheck` and `bun run build` exit 0; `bun run lint` and `bun test src scripts` fail with the same pre-existing errors confirmed present on `main` before this change via `git stash` (10 unrelated Biome errors in `scripts/install-page.html`/a test file; one flaky `import.meta.glob`-under-`bun test` DB-bootstrap cascade) — not a regression from this plan; one deviation from the plan's literal Step 2 code: Biome's formatter collapses multi-space JSX text runs, which would have silently broken the `--help` block's column alignment, so padding/gaps are wrapped in JS string literals (`{"importLeague "}` etc.) instead of raw JSX text spaces |
| 092  | Rewrite README to the locked "one screen" concept, split migrate table to docs/migrate.md | P1 | M | — | DONE `f277ffc` (2026-08-26, not pushed); README rewritten to Step 1's exact text (131→60 lines); the full `## Migrating your league` section (intro, all three per-source subsections, and the 5-column comparison table) moved to new `docs/migrate.md` verbatim — verified byte-identical via `diff` against the `101cd0c` original (only the `##`→`#` heading level differs, as specified); `docs/images/hero-banner.png` (1200x260, already produced) referenced correctly and committed; `bun run typecheck` and `bun run build` both exit 0; `bun run lint` fails with the same pre-existing 10 errors it has on `main` before this change (confirmed via `git stash`), none touching README.md/docs/ — not a regression from this plan |
