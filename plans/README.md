...
| 091  | Add a real `/about` marketing page, built from the locked mock | P2 | M | — | TODO — planned at `101cd0c`; implements the user-approved hdls.tools-inspired landing page mock (Console tokens, real Claude/OpenAI icons, `--help`-style features block), using the app's real Tailwind semantic tokens instead of the mock's hardcoded hex, forcing `data-skin="console"` on mount/restoring on unmount, and keeping only the one real twice-(now thrice-)verified demo exchange rather than the mock's second fabricated turn |
