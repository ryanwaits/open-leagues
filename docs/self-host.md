...
| `OPENLEAGUES_SPLITS_SOURCE` | Optional, off by default. Comma-separated: `actionnetwork` (consensus, history from the 2023 season), `dknetwork` (DraftKings' own handle/bet share, current slate), `wiseguyteam` (multi-book aggregate, book named, current slate). Recommended for a box that runs the lab: `actionnetwork,dknetwork,wiseguyteam`. All undocumented web endpoints; every pulled week is kept. Unset and `getBettingSplits` returns no games. |
| `OPENLEAGUES_MODE` | Unset = `substrate`, the safe default and the public host's shape: receipts, open data, the lab, and `/api/mcp` for anyone — no accounts, tokens, or leagues; the auth API returns 404 and sign-in/league routes show a notice. `league` = the whole product. docker-compose sets `league`; so does `bun run dev`. |
