...
[context-prompt.md](../../src/lib/agent/context-prompt.md). Start with
...
6. **Yahoo / NFL.com:** commit-only on the PWA `/import` page; no direct
   MCP path. Point them there. Never ask them to paste cookies into chat;
   never echo cookies. Never fetch `fantasy.nfl.com` HTML.
